/**
 * `executeScript` returns whatever DOM the test staged for that tab. Staging is
 * a queue, so a test can walk a tab through several steps and assert that the
...
export class FakeChromeScripting {
  ...
  stage(tabId: number, ...results: string[]): void {
    const queue = this.#staged.get(tabId) ?? []
    queue.push(...results)
    this.#staged.set(tabId, queue)
  }

  async executeScript({ target }: { target: { tabId: number } }): Promise<{ result: string }[]> {
