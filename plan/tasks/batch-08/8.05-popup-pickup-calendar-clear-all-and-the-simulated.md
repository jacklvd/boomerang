...
conflicts_with_raw: "Tasks 8.3, 8.4 (the popup shell and route table in `extension/entrypoints/popup/`)"
parallel_with: []
parallel_with_raw: "—"
...
**Conflicts with:** Tasks 8.3, 8.4 (the popup shell and route table in `extension/entrypoints/popup/`)
**Parallel with:** —
**Package:** `extension/entrypoints/popup`
...
8. Unit tests: the confirmation shows the address and carrier and requires consent; consent is
   recorded before the booking call is issued; **a `MOCK_CONFIRMATION_PREFIX` confirmation number
   renders the simulated marker and an unprefixed real-carrier confirmation number does not**; the
   collection copy names a day and matches no window phrasing; an ineligible result offers
   alternatives with prices; cancel renders collected as its own outcome; a failed calendar tab
   still offers the `.ics`; clear-all with an unsettled pickup warns and double-confirms.
