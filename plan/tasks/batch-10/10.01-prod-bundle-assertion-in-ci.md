...
3. Also assert the built manifest's permission array is exactly `activeTab`, `scripting`, `storage`,
   that `<all_urls>` appears nowhere, and that `externally_connectable.matches` contains exactly the
   concrete production `DASHBOARD_ORIGIN` (decision D28) — the same assertions as Task 1.2, but
   against the **built** artefact, which is the one that ships.
4. Wire it into the `build` script so it cannot be skipped by forgetting a separate command.
5. Reference: Low-Level Design §7.2; requirements FR-3.6.3, FR-3.7.1, NFR-6.5; decisions D20, D28.
