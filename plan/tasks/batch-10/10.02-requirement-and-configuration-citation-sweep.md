...
track_scope: null
title: "Requirement and configuration citation sweep"
kind: "implementation"
...
requirements_covered_raw: "— (this task verifies every other task's citations; FR-3.6.2 is the one allowlisted exemption)"
sections_covered: []
status: "not_started"
---
#### Task 10.2: Requirement and configuration citation sweep
...
7. **One known, deliberate exemption**, in an explicit allowlist with the reason inline:
   - **FR-3.6.2** — the phase-1 landing page and install funnel remain outside this implementation
     plan because `client/` is explicitly excluded from the implementation design.
   FR-3.6.3 is in scope through Tasks 5.6, 8.7, and 9.7 and must not be allowlisted.
...
**Verification:**
- `bash scripts/citation-sweep.sh` — exits zero with FR-3.6.2 listed as the only exemption and
